...
class Medicale_Controller {
    ...
    public function AddProducte() {
       ...
       if (empty($product_name) || empty($product_lot) || empty($date_expiration) || $quantity <= 0) {
        $_SESSION['error_message'] = "All fields are required, and the quantity must be greater than zero.";
        self::redirectTo('../../templates/views/Préparateur/dashboard.php?error_empty');
        }
        ...
        if ($isSaved) {
            ...
            self::redirectTo('../../templates/views/Préparateur/dashboard.php?status=success');
        } else {
            $_SESSION['error_message'] = "Failed to register the product. Please try again.";
            self::redirectTo('../../templates/views/Préparateur/dashboard.php?status=failed');
        }
    }
}
