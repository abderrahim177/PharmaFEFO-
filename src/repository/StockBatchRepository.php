...
class TotalLots {
    ...
    public function getLotsCriticiteStats() : array {
        try {
            $query = "SELECT 
                        ...
                        COALESCE(SUM(CASE WHEN expiration_date > CURDATE() + INTERVAL 90 DAY THEN 1 ELSE 0 END), 0) AS total_vert
                      FROM lots
                      WHERE quantity > 0";
    ...
    {
        try {
            $query = 'SELECT p.name AS product_name, l.lot_number, l.expiration_date, l.quantity, l.status 
                      FROM products p
